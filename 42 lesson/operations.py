...
def add_data():
    ...
    with SessionMaker() as session:
        ...
        students = [Student(first_name=fake.first_name(), last_name=fake.last_name(),
                            courses=sample(courses, k=randint(1, len(courses))),
                            profile=StudentProfile(phone_number=fake.phone_number(), email=fake.email()))
                    for _ in range(7)]
...
def get_student_profile_by_id(user_id: int):
    with SessionMaker() as session:
        stmt = select(Student).where(Student.id == user_id)
        student = session.scalars(stmt).one_or_none()
        if student:
            print(f"ID {student.id} | {student.first_name} {student.last_name}'s profile data:\n"
                  f"Phone number: {student.profile.phone_number}, Email: {student.profile.email}")
